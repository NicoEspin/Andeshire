...
const CandidateScoreboard = () => {
  const t = useTranslations("CandidateDetail.Scoreboards");
  const dispatch = useDispatch<AppDispatch>();

  const { candidateName, scoreboards, availableTemplates, loading, loaded } =
    useSelector((state: RootState) => state.candidateScoreboards);
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center border-b pb-4">
          <h2 className="text-2xl font-semibold">
            {t("Header", { name: candidateName || "Candidato" })}
          </h2>
            ...
            <Card key={scoreboard.id}>
              <CardHeader className="flex flex-row justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    {t("Updated")}: {scoreboard.updated_at} | {t("Recruiter")}:{" "}
                    {scoreboard.recruiter_name}
                  </CardDescription>
                </div>
              ...
              <CardFooter>
                ...
                >
                  ...
                  ></span>
                  {scoreboard.to_complete ? t("Pending") : t("Completed")}
                </div>
              </CardFooter>
            </Card>
          ))}
